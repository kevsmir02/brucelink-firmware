...
| **`crashlog` / `-clear` / `-selftest`** | **Verified end to end 2026-07-30, device 1C:DB:D4:5E:D7:39, app elf `79fc138cd`.** Reads the ELF core dump a panic stores in flash, over BLE, with **no process holding `/dev/ttyACM0`** (`fuser`: no holders). `-selftest` aborts deliberately so the crash path has a *known* expected answer instead of a race: the recovered backtrace decoded to `crash_commands.cpp:35` → `SimpleCLI::parse` → `handleSerialCommands` (`serialcmds.cpp:87`) → `_serialCmdsTaskLoop` (`:100`), i.e. exactly the abort that was asked for, on the BLE dispatch path. `task=serialcmds`, `depth=9`, `corrupted=no`, `match=yes`. Boot line confirmed on console: `[E][crash_commands.cpp:93] reportBootCrashState(): [BOOT] reset_reason=panic(4) coredump=stored`. `-clear` → `crash: cleared`, then `crash: none stored`. **The panic handler's `ELF file SHA256: 79fc138cd` is byte-identical to `crashlog`'s `running_elf=`**, which is what makes the `match=` verdict trustworthy |
| **`webui` start gate — `webui -bg` / `-selftest`** | **Verified on hardware 2026-07-31, device 1C:DB:D4:5E:D7:39, ELF `8a30e3153`.** `webui` now reports a real outcome instead of always claiming success, and `[CLI] Result:` from this verb can be trusted. Baseline `webui -bg` → `started mode=ap dma=6900 required=15360 tcp_state=1`, AP confirmed on air by `nmcli` (BSSID `1E:DB:D4:5E:D7:38`). **All three refusal gates induced**: `wifi_bringup_failed` (dma 7412), `low_dma_pre_alloc` (dma 2676), `not_listening` (via `-selftest`, dma 6900). Each produced `[CLI] Result: FALSE`, a console `log_e` and an event frame at **`level:"err"`** — the first real producers of that level. `low_dma_pre_alloc` and `not_listening` both **unwound cleanly**: no AP in a rescan taken >15 s later, heap back to 63,127/dma 17,396 and 58,147/dma 19,444. Gate D freed a **live `AsyncWebServer` without panicking**. ⚠️ `wifi_bringup_failed`'s unwind **panicked in the WiFi driver on 1 of 3 runs** — ISSUE-43, open. ⚠️ ISSUE-12's marginal class is **not** fixed |
...
| `reverseshell` — **AP leak fixed, not yet re-tested** | The verb works end to end and exits cleanly (`cedad77f` closed the heap-corruption reboot, ISSUE-38). The exit never brought the radio down, so `BruceShell` kept broadcasting WPA2 and held ~63 KB — confirmed 2026-07-30 by two scans plus a successful association. **`wifiDisconnect()` now runs on all four exits**, but this is **code-verified only**: the verb blocks the serial task and binds port 80, so verifying it needs an operator at the board to press LEFT+RIGHT — **twice**, because the first press on a dimmed screen is swallowed (`interface.cpp:119-123`). ⚠️ **Citation corrected 2026-07-31:** this row previously pinned the fix to ELF `46d975be7d38f128`, which is stale — that build is long superseded and the fix has shipped in every build since. The current build is `8a30e3153a15326a`. What is still outstanding is the *operator test*, not the code | ISSUE-39 |
...
| Give a `js` script ~2 s before `webui` | The interpreter task's 16 KB stack is reclaimed by the FreeRTOS **idle task**, not at `vTaskDelete`. Start the WebUI inside that window and AsyncTCP cannot allocate; wait, and it is fine (ISSUE-17). **Measured 2026-07-31: one BLE round trip is already long enough** — `free` before and after a `js` run read identically (80,751 / dma 31,732), with only `minEver` dipping to 56,539. The window is only reachable by writing both verbs in a *single* BLE packet, which is how ISSUE-12's gates were induced deliberately. Ordinary sequential dispatch is safe. |
