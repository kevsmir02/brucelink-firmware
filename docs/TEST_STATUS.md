...
| `reverseshell` — **AP leak fixed, not yet re-tested** | The verb works end to end and exits cleanly (`cedad77f` closed the heap-corruption reboot, ISSUE-38). The exit never brought the radio down, so `BruceShell` kept broadcasting WPA2 and held ~63 KB — confirmed 2026-07-30 by two scans plus a successful association. **`wifiDisconnect()` now runs on all four exits** (ELF `46d975be7d38f128`), but this is **code-verified only**: the verb blocks the serial task and binds port 80, so verifying it needs an operator at the board to press LEFT+RIGHT | ISSUE-39 |
